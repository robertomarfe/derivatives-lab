...
export function fairForwardPrice(S0, r, delta, T) {
  V.positive('S0', S0); V.finite('r', r); V.finite('delta', delta); V.nonNegative('T', T);
...
export function diagnoseForwardArbitrage(S0, marketForward, r, delta, T, tolerance=1e-10) {
  V.positive('Market forward', marketForward); V.nonNegative('tolerance', tolerance);
...
// ---------- Payoffs ----------
export function callPayoff(ST,K){V.nonNegative('S_T',ST);V.positive('K',K);return Math.max(ST-K,0)}
export function putPayoff(ST,K){V.nonNegative('S_T',ST);V.positive('K',K);return Math.max(K-ST,0)}
export function legPayoff(ST,leg){
  V.nonNegative('S_T',ST); V.finite('Leg quantity', leg.quantity); V.oneOf('Instrument',leg.instrument,['call','put','underlying']);
  if(leg.instrument==='underlying') return leg.quantity*ST;
  if(leg.strike==null) throw new Error('Option legs require a strike.'); V.positive('Strike',leg.strike);
  return leg.quantity*(leg.instrument==='call'?callPayoff(ST,leg.strike):putPayoff(ST,leg.strike));
}
export function portfolioPayoff(ST,legs){if(!Array.isArray(legs)||!legs.length)throw new Error('Portfolio must contain at least one leg.');return legs.reduce((s,l)=>s+legPayoff(ST,l),0)}
export function strategyPreset(name,Klow,KATM,Khigh,ratioN=2){
  V.positive('K_low',Klow);V.positive('K_ATM',KATM);V.positive('K_high',Khigh);V.positiveInteger('Ratio n',ratioN);
...
export const terminalProfitFromNetCost=(payoff,netCost,r,T)=>{V.finite('Payoff',payoff);V.finite('Net cost',netCost);V.finite('r',r);V.nonNegative('T',T);return payoff-netCost*Math.exp(r*T)};
...
export function marginAccountPath(prices, contractMultiplier, nContracts, marginRate, r, periodsPerYear, maintenanceRatio, position='long'){
  V.positiveArray('Futures prices',prices,{minLength:2}); V.positive('Contract multiplier',contractMultiplier); V.positiveInteger('Number of contracts',nContracts); V.positive('Initial margin rate',marginRate); V.finite('r',r); V.positiveInteger('Periods/year',periodsPerYear); V.bounded('Maintenance / initial margin',maintenanceRatio,0,1); V.oneOf('Position',position,['long','short']);
...
export const minimumVarianceHedgeRatio=(rho,sigmaS,sigmaF)=>{V.probabilityLikeCorrelation('Correlation rho',rho);V.nonNegative('sigma_S',sigmaS);V.positive('sigma_F',sigmaF);return rho*sigmaS/sigmaF};
export const hedgeEffectiveness=rho=>{V.probabilityLikeCorrelation('Correlation rho',rho);return rho*rho};
export const optimalNumberFuturesContracts=(h,QA,QF)=>{V.finite('Hedge ratio',h);V.positive('Q_A',QA);V.positive('Q_F',QF);return Math.abs(h)*QA/QF};
export function crossHedge(rho,sigmaS,sigmaF,QA,QF,exposure='will_purchase'){
  V.oneOf('Exposure',exposure,['will_purchase','own_or_will_sell']);
  const h=minimumVarianceHedgeRatio(rho,sigmaS,sigmaF), n=optimalNumberFuturesContracts(h,QA,QF), exposureSign=exposure==='will_purchase'?1:-1, signedPosition=exposureSign*Math.sign(h);
  const recommendedFuturesPosition=signedPosition>0?'Long futures':signedPosition<0?'Short futures':'No futures position';
  return {rho,sigmaS,sigmaF,hedgeRatio:h,hedgeEffectiveness:hedgeEffectiveness(rho),QA,QF,optimalContracts:n,roundedContracts:Math.floor(n+0.5),recommendedFuturesPosition,signedContractPosition:signedPosition*n};
}
export function estimateCrossHedgeFromChanges(deltaS,deltaF){
  V.finiteArray('Delta S',deltaS,{minLength:2});V.finiteArray('Delta F',deltaF,{minLength:2});V.sameLength({deltaS,deltaF});
  const n=deltaS.length, ms=mean(deltaS), mf=mean(deltaF), sigmaS=sampleStd(deltaS), sigmaF=sampleStd(deltaF);
  if(!(sigmaS>0&&sigmaF>0))throw new Error('Cross-hedge regression requires positive sample volatility for both Delta S and Delta F.');
...
export function d1d2(S,K,r,sigma,T,delta=0){
  V.positive('S',S); V.positive('K',K); V.finite('r',r); V.finite('delta',delta); V.positive('sigma',sigma); V.positive('T',T);
...
export function blackScholesPrice(S,K,r,sigma,T,delta=0,optionType='call'){
  V.positive('S',S); V.positive('K',K); V.finite('r',r); V.finite('delta',delta); V.nonNegative('sigma',sigma); V.nonNegative('T',T); V.oneOf('Option type',optionType,['call','put']);
  ...
  const price=optionType==='call'?S*dq*normCdf(d1)-K*dr*normCdf(d2):K*dr*normCdf(-d2)-S*dq*normCdf(-d1);return Math.max(0,price);
}
export function blackScholesGreeks(S,K,r,sigma,T,delta=0,optionType='call'){
  V.positive('T',T); V.positive('sigma',sigma); V.oneOf('Option type',optionType,['call','put']);
...
// ---------- Binomial ----------
export function lectureTreeParameters(r,delta,sigma,h){V.finite('r',r);V.finite('delta',delta);V.positive('sigma',sigma);V.positive('h',h);const u=Math.exp((r-delta)*h+sigma*Math.sqrt(h)),d=Math.exp((r-delta)*h-sigma*Math.sqrt(h)),g=Math.exp((r-delta)*h); if(!(d<g&&g<u))throw new Error('No-arbitrage condition violated.'); return {u,d,p:(g-d)/(u-d)};}
export function priceBinomial(S0,K,r,sigma,T,n,delta=0,optionType='call',style='european'){
  V.positive('S0',S0);V.positive('K',K);V.finite('r',r);V.positive('sigma',sigma);V.positive('T',T);V.positiveInteger('n',n);V.finite('delta',delta);V.oneOf('Option type',optionType,['call','put']);V.oneOf('Exercise style',style,['european','american']);
...
// ---------- Implied volatility ----------
export function noArbitrageBounds(S,K,r,T,delta=0,optionType='call'){V.positive('S',S);V.positive('K',K);V.finite('r',r);V.positive('T',T);V.finite('delta',delta);V.oneOf('Option type',optionType,['call','put']);const ds=S*Math.exp(-delta*T),dk=K*Math.exp(-r*T);return optionType==='call'?[Math.max(ds-dk,0),ds]:[Math.max(dk-ds,0),dk];}
export function impliedVolatility(marketPrice,S,K,r,T,delta=0,optionType='call'){
  V.nonNegative('Market option price',marketPrice);
...
export function logContractValue(S0,K,r,sigma,T){V.positive('S0',S0);V.positive('K',K);V.finite('r',r);V.nonNegative('sigma',sigma);V.positive('T',T);return Math.exp(-r*T)*(Math.log(S0/K)+(r-0.5*sigma*sigma)*T)}
export function varianceFromForwardLogContract(L0,r,T){V.finite('Log-contract value',L0);V.finite('r',r);V.positive('T',T);return 2*Math.exp(r*T)*(-L0)/T}
export function strikeSpacings(k){V.strictlyIncreasing('Strikes',k);V.positiveArray('Strikes',k,{minLength:2});const n=k.length,dk=Array(n);dk[0]=k[1]-k[0];dk[n-1]=k[n-1]-k[n-2];for(let i=1;i<n-1;i++)dk[i]=0.5*(k[i+1]-k[i-1]);return dk;}
export function discreteVixVariance(strikes,puts,calls,forwardPrice,r,T){V.positiveArray('Strikes',strikes,{minLength:2});V.strictlyIncreasing('Strikes',strikes);V.nonNegativeArray('Put prices',puts,{minLength:2});V.nonNegativeArray('Call prices',calls,{minLength:2});V.sameLength({strikes,puts,calls});V.positive('Forward price',forwardPrice);V.finite('r',r);V.positive('T',T);let idx=-1;for(let i=0;i<strikes.length;i++)if(strikes[i]<forwardPrice)idx=i;if(idx<0)throw new Error('Need a strike below forward.');const K0=strikes[idx],dk=strikeSpacings(strikes),q=strikes.map((K,i)=>K<=K0?puts[i]:calls[i]),contribution=strikes.map((K,i)=>dk[i]*q[i]/(K*K));let variance=2*Math.exp(r*T)/T*contribution.reduce((a,b)=>a+b,0)-(1/T)*(forwardPrice/K0-1)**2;if(variance<-1e-10)throw new Error('Negative variance from strip.');variance=Math.max(0,variance);const vol=Math.sqrt(variance);return {variance,volatility:vol,vixLevel:100*vol,K0,deltaK:dk,selectedOptionPrices:q,contribution};}

// ---------- Heston ----------
export const fellerMargin=(k,theta,sigma)=>{V.positive('k',k);V.nonNegative('theta',theta);V.positive('sigma',sigma);return 2*k*theta-sigma*sigma};
export function hestonCharacteristic(u,S,r,T,nu0,k,theta,sigma,rho){
  V.positive('S',S);V.finite('r',r);V.positive('T',T);V.nonNegative('nu0',nu0);V.positive('k',k);V.nonNegative('theta',theta);V.positive('sigma',sigma);V.probabilityLikeCorrelation('rho',rho);
...
export function hestonCallPrice(S,K,r,T,nu0,k,theta,sigma,rho,lambdaV=0,integrationUpper=100){
  V.positive('S',S);V.positive('K',K);V.finite('r',r);V.positive('T',T);V.nonNegative('nu0',nu0);V.positive('k',k);V.nonNegative('theta',theta);V.positive('sigma',sigma);V.probabilityLikeCorrelation('rho',rho);V.positive('Integration upper bound',integrationUpper);
...
export const jumpCompensator=(m,delta)=>{V.finite('m',m);V.nonNegative('Jump-size standard deviation delta',delta);return Math.exp(m+0.5*delta*delta)-1};
function poissonTerms(mu,tailTol=1e-12){V.nonNegative('Poisson mean',mu);V.bounded('Tail tolerance',tailTol,0,1,{openMin:true,openMax:true});const out=[];let w=Math.exp(-mu),sum=0,j=0;while(j<10000){out.push(w);sum+=w;if(1-sum<tailTol&&j>mu)break;j++;w*=mu/j;}return {weights:out,omitted:Math.max(0,1-sum)};}
export function mertonCallPrice(S,K,r,T,sigma,lam,m,delta,formulation='eq35',tailTol=1e-12){V.positive('S',S);V.positive('K',K);V.finite('r',r);V.positive('T',T);V.nonNegative('Diffusion sigma',sigma);V.nonNegative('Jump intensity lambda',lam);V.finite('m',m);V.nonNegative('Jump-size standard deviation delta',delta);V.oneOf('Formulation',formulation,['eq35','eq36']);const kj=jumpCompensator(m,delta);if(lam===0){const bs=blackScholesPrice(S,K,r,sigma,T,0,'call');return {callPrice:bs,terms:[{j:0,weight:1,conditionalSigma:sigma,conditionalSpot:S,conditionalRate:r,conditionalBsPrice:bs,contribution:bs}],omittedProbabilityMass:0,jumpCompensatorK:kj};}const intensity=formulation==='eq35'?lam:lam*(1+kj),{weights,omitted}=poissonTerms(intensity*T,tailTol),terms=[];let total=0;for(let j=0;j<weights.length;j++){const weight=weights[j],sigmaJ=Math.sqrt(sigma*sigma+j*delta*delta/T);let spotJ,rateJ;if(formulation==='eq35'){spotJ=S*Math.exp(j*m+0.5*j*delta*delta-lam*T*Math.exp(m+0.5*delta*delta)+lam*T);rateJ=r;}else{spotJ=S;rateJ=r-lam*kj+j*Math.log(1+kj)/T;}const bs=blackScholesPrice(spotJ,K,rateJ,sigmaJ,T,0,'call'),contribution=weight*bs;total+=contribution;terms.push({j,weight,conditionalSigma:sigmaJ,conditionalSpot:spotJ,conditionalRate:rateJ,conditionalBsPrice:bs,contribution});}return {callPrice:total,terms,omittedProbabilityMass:omitted,jumpCompensatorK:kj};}

// ---------- Merton structural credit ----------
export function mertonD1D2(V0,D,r,sigmaV,T){V.positive('V0',V0);V.positive('D',D);V.finite('r',r);V.positive('sigma_V',sigmaV);V.positive('T',T);const root=Math.sqrt(T),d1=(Math.log(V0/D)+(r+0.5*sigmaV*sigmaV)*T)/(sigmaV*root);return [d1,d1-sigmaV*root];}
...
export function equityVolatilityFromFirm(V0,E0,D,r,sigmaV,T){V.positive('E0',E0);const[d1]=mertonD1D2(V0,D,r,sigmaV,T);return (V0/E0)*normCdf(d1)*sigmaV;}
export function riskNeutralDefaultProbability(V0,D,r,sigmaV,T){const[,d2]=mertonD1D2(V0,D,r,sigmaV,T);return normCdf(-d2);}
export function distanceToDefault(V0,D,alpha,sigmaV,T){V.positive('V0',V0);V.positive('D',D);V.finite('alpha',alpha);V.positive('sigma_V',sigmaV);V.positive('T',T);return (Math.log(V0/D)+(alpha-0.5*sigmaV*sigmaV)*T)/(sigmaV*Math.sqrt(T));}
...
export function inferFirmValueFromEquity(E0,sigmaE,D,r,T){V.positive('Observed equity E0',E0);V.positive('Observed equity volatility sigma_E',sigmaE);V.positive('Debt face D',D);V.finite('r',r);V.positive('T',T);let x=Math.log(E0+D*Math.exp(-r*T)),y=Math.log(Math.max(1e-4,sigmaE*E0/Math.exp(x)));
...
export function simulateGbmPaths(S0,drift,sigma,T,nPaths,nSteps,seed=12345){V.positive('S0',S0);V.finite('Drift',drift);V.nonNegative('sigma',sigma);V.positive('T',T);V.positiveInteger('Paths',nPaths);V.positiveInteger('Steps',nSteps);V.finite('Seed',seed);const rng=makeRng(seed),dt=T/nSteps,times=Array.from({length:nSteps+1},(_,i)=>i*dt),paths=[];for(let p=0;p<nPaths;p++){const row=[S0];let logS=Math.log(S0);for(let t=0;t<nSteps;t++){logS+=(drift-0.5*sigma*sigma)*dt+sigma*Math.sqrt(dt)*rng.normal();row.push(Math.exp(logS));}paths.push(row);}return {times,paths};}
export function monteCarloEuropeanPrice(S0,K,r,sigma,T,delta,optionType,nPaths,seed=12345){V.positive('S0',S0);V.positive('K',K);V.finite('r',r);V.nonNegative('sigma',sigma);V.positive('T',T);V.finite('delta',delta);V.oneOf('Option type',optionType,['call','put']);V.positiveInteger('Paths',nPaths);V.finite('Seed',seed);const rng=makeRng(seed),disc=Math.exp(-r*T),xs=[];for(let i=0;i<nPaths;i++){const z=rng.normal(),ST=S0*Math.exp((r-delta-0.5*sigma*sigma)*T+sigma*Math.sqrt(T)*z),pay=optionType==='call'?Math.max(ST-K,0):Math.max(K-ST,0);xs.push(disc*pay);}const estimate=mean(xs),se=sampleStd(xs)/Math.sqrt(nPaths);return {estimate,standardError:se,ci95Low:estimate-1.96*se,ci95High:estimate+1.96*se,exactBlackScholes:blackScholesPrice(S0,K,r,sigma,T,delta,optionType)};}
...
export function simulateHedging(charge,K,T,S0,r,mu,sigma,nPaths,nSteps,TC=0,gammaHedge=false,K2=115,T2=1,seed=12345){V.finite('Option charge',charge);V.positive('K',K);V.positive('T',T);V.positive('S0',S0);V.finite('r',r);V.finite('mu',mu);V.positive('sigma',sigma);V.positiveInteger('Paths',nPaths);V.positiveInteger('Steps',nSteps);V.nonNegative('Transaction-cost rate',TC);V.positive('Hedge-option strike K2',K2);V.requireAtLeast('Hedge-option maturity T2',T2,T);V.finite('Seed',seed);const rng=makeRng(seed),dt=T/nSteps,growth=Math.exp(r*dt),GH=gammaHedge?1:0,final=[];for(let p=0;p<nPaths;p++){let S=S0;const g1=blackScholesGreeks(S0,K,r,sigma,T,0,'call'),p20=blackScholesPrice(S0,K2,r,sigma,T2,0,'call'),g2=blackScholesGreeks(S0,K2,r,sigma,T2,0,'call');let q2=GH*clamp(g1.gamma/g2.gamma,-100,100),qS=g1.delta-q2*g2.delta,balance=charge-qS*S0-GH*q2*p20-TC*(Math.abs(qS)*S0+GH*Math.abs(q2)*p20);for(let t=0;t<nSteps-1;t++){S*=Math.exp((mu-0.5*sigma*sigma)*dt+sigma*Math.sqrt(dt)*rng.normal());const tau1=T-(t+1)*dt,tau2=T2-(t+1)*dt,b1=bsArrays(K,tau1,S,r,sigma),b2=bsArrays(K2,tau2,S,r,sigma),newQ2=GH*clamp(Math.abs(b2.gamma)>1e-15?b1.gamma/b2.gamma:0,-100,100),newQS=b1.delta-newQ2*b2.delta,stockCost=(newQS-qS)*S,optionCost=GH*(newQ2-q2)*b2.price,tc=TC*(Math.abs(newQS-qS)*S+GH*Math.abs(newQ2-q2)*b2.price);balance=growth*balance-stockCost-optionCost-tc;qS=newQS;q2=newQ2;}S*=Math.exp((mu-0.5*sigma*sigma)*dt+sigma*Math.sqrt(dt)*rng.normal());const p2f=bsArrays(K2,T2-T,S,r,sigma).price;balance=growth*balance+qS*S+GH*q2*p2f-Math.max(S-K,0);final.push(balance);}return {finalBalance:final,mean:mean(final),std:sampleStd(final),percentile5:quantile(final,0.05)};}
export function simulateHedgingPath(charge,K,T,S0,r,mu,sigma,nSteps,TC=0,gammaHedge=false,K2=115,T2=1,seed=12345){
  V.finite('Option charge',charge);V.positive('K',K);V.positive('T',T);V.positive('S0',S0);V.finite('r',r);V.finite('mu',mu);V.positive('sigma',sigma);V.positiveInteger('Steps',nSteps);V.nonNegative('Transaction-cost rate',TC);V.positive('Hedge-option strike K2',K2);V.requireAtLeast('Hedge-option maturity T2',T2,T);V.finite('Seed',seed);
  const rng=makeRng(seed),dt=T/nSteps,growth=Math.exp(r*dt),GH=gammaHedge?1:0;
  const times=Array.from({length:nSteps+1},(_,i)=>i*dt),stockPath=[S0],stockQuantity=[],option2Quantity=[],bankBalance=[],transactionCostPaid=[];
  let S=S0;
  const g1=blackScholesGreeks(S0,K,r,sigma,T,0,'call'),p20=blackScholesPrice(S0,K2,r,sigma,T2,0,'call'),g2=blackScholesGreeks(S0,K2,r,sigma,T2,0,'call');
  let q2=GH*clamp(g1.gamma/g2.gamma,-100,100),qS=g1.delta-q2*g2.delta;
  const initialTc=TC*(Math.abs(qS)*S0+GH*Math.abs(q2)*p20);
  let balance=charge-qS*S0-GH*q2*p20-initialTc;
  stockQuantity.push(qS);option2Quantity.push(q2);bankBalance.push(balance);transactionCostPaid.push(initialTc);
  for(let t=0;t<nSteps-1;t++){
    S*=Math.exp((mu-0.5*sigma*sigma)*dt+sigma*Math.sqrt(dt)*rng.normal());stockPath.push(S);
    const tau1=T-(t+1)*dt,tau2=T2-(t+1)*dt,b1=bsArrays(K,tau1,S,r,sigma),b2=bsArrays(K2,tau2,S,r,sigma);
    const newQ2=GH*clamp(Math.abs(b2.gamma)>1e-15?b1.gamma/b2.gamma:0,-100,100),newQS=b1.delta-newQ2*b2.delta;
    const stockCost=(newQS-qS)*S,optionCost=GH*(newQ2-q2)*b2.price,tc=TC*(Math.abs(newQS-qS)*S+GH*Math.abs(newQ2-q2)*b2.price);
    balance=growth*balance-stockCost-optionCost-tc;qS=newQS;q2=newQ2;
    stockQuantity.push(qS);option2Quantity.push(q2);bankBalance.push(balance);transactionCostPaid.push(tc);
  }
  S*=Math.exp((mu-0.5*sigma*sigma)*dt+sigma*Math.sqrt(dt)*rng.normal());stockPath.push(S);
  const p2f=bsArrays(K2,T2-T,S,r,sigma).price,finalBalance=growth*balance+qS*S+GH*q2*p2f-Math.max(S-K,0);
  stockQuantity.push(qS);option2Quantity.push(q2);bankBalance.push(finalBalance);transactionCostPaid.push(0);
  return {times,stockPath,stockQuantity,option2Quantity,bankBalance,transactionCostPaid,finalBalance};
}

export function simulateHestonPaths(S0,r,T,nu0,k,theta,sigma,rho,nPaths=1000,nSteps=126,seed=12345){V.positive('S0',S0);V.finite('r',r);V.positive('T',T);V.nonNegative('nu0',nu0);V.positive('k',k);V.nonNegative('theta',theta);V.positive('sigma',sigma);V.probabilityLikeCorrelation('rho',rho);V.positiveInteger('Paths',nPaths);V.positiveInteger('Steps',nSteps);V.finite('Seed',seed);const rng=makeRng(seed),dt=T/nSteps,times=Array.from({length:nSteps+1},(_,i)=>i*dt),stockPaths=[],variancePaths=[],ek=Math.exp(-k*dt),C=4*k/(sigma*sigma*(1-ek)),df=4*k*theta/(sigma*sigma),orth=Math.sqrt(Math.max(0,1-rho*rho));for(let p=0;p<nPaths;p++){let v=nu0,logS=Math.log(S0),srow=[S0],vrow=[v];for(let t=0;t<nSteps;t++){const nonc=C*ek*v,vnew=rng.noncentralChiSquare(df,nonc)/C,iv=0.5*dt*(v+vnew),y=rng.normal();logS=logS+r*dt+(rho/sigma)*(vnew-v-k*theta*dt)+(k*rho/sigma-0.5)*iv+orth*Math.sqrt(Math.max(iv,0))*y;v=vnew;srow.push(Math.exp(logS));vrow.push(v);}stockPaths.push(srow);variancePaths.push(vrow);}return {times,stockPaths,variancePaths};}
export function simulateMertonPaths(S0,r,T,sigma,lam,m,delta,nPaths=1000,nSteps=126,seed=12345){V.positive('S0',S0);V.finite('r',r);V.positive('T',T);V.nonNegative('sigma',sigma);V.nonNegative('Jump intensity lambda',lam);V.finite('m',m);V.nonNegative('Jump-size standard deviation delta',delta);V.positiveInteger('Paths',nPaths);V.positiveInteger('Steps',nSteps);V.finite('Seed',seed);const rng=makeRng(seed),dt=T/nSteps,times=Array.from({length:nSteps+1},(_,i)=>i*dt),stockPaths=[],jumpCounts=[],kj=jumpCompensator(m,delta);for(let p=0;p<nPaths;p++){let logS=Math.log(S0),cum=0,srow=[S0],jrow=[0];for(let t=0;t<nSteps;t++){const n=rng.poisson(lam*dt),jump=n*m+Math.sqrt(n)*delta*rng.normal();logS+=(r-lam*kj-0.5*sigma*sigma)*dt+sigma*Math.sqrt(dt)*rng.normal()+jump;cum+=n;srow.push(Math.exp(logS));jrow.push(cum);}stockPaths.push(srow);jumpCounts.push(jrow);}return {times,stockPaths,jumpCounts};}
