...
export function renderHeston(root){
  ...
  tabbed('heston',root,[['dynamics','Dynamics'],['pricing','Pricing'],['simulation','Simulation']],(t,r)=>{
    const controls=controlGrid(
      field('$S_0$','he-S',s.S,{min:.000001})+field('$r$','he-r',s.r,{step:.005})+field('$\\nu_0$','he-nu',s.nu0,{step:.005,min:0})+
      field('$k$','he-k',s.k,{step:.1,min:.000001})+field('$\\theta$','he-theta',s.theta,{step:.005,min:0})+
    ...
    if(t==='dynamics'){
      ...
      const update=safeUpdate(r,()=>{
        ...
        // Validate the full parameter block, not only the three quantities entering the Feller margin.
      ...
      qs('#he-price',r).onclick=safeUpdate(r,()=>{
        ...
        try{
          ...
          lineChart(qs('#he-smile',r),[{name:'Heston implied volatility',x:K,y:iv}],{xLabel:'Strike K',yLabel:'Implied volatility (%)'});
